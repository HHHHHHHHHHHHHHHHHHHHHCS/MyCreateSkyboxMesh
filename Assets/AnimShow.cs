...
public class AnimShow : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        if (vData != null)
            foreach (var item in vData)
            {
                ...
                Gizmos.DrawSphere(new Vector3(item.x, item.y, item.z), 0.01f);
            }
    }
    ...
    private WaitForSeconds wsp25 = new WaitForSeconds(0.25f);
    private WaitForSeconds wsp05 = new WaitForSeconds(0.05f);


    private List<CubemapSkyboxVertex> vData;
    ...
    public IEnumerator Create()
    {
        ...
        vData = new List<CubemapSkyboxVertex>(cap);
        ...
        yield return StartCoroutine(SubdivideSkybox(vData, NUM_FULL_SUBDIVISIONS));
    ...
    private IEnumerator SubdivideSkybox(List<CubemapSkyboxVertex> vertexData, int subdivisions)
    {
